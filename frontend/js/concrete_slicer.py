...
class ConcreteSlicer:
    ...
    def __init__(self, stl_path, custom_settings=None):
        ...
        # Center the mesh to match frontend visualization
        # This ensures toolpaths align with the displayed model
    ...
    def slice_to_layers(self):
        """Slice the mesh into horizontal layers using trimesh's built-in slicer"""
        ...
        if num_layers > 500:
            print(f"   ⚠️  Warning: Model has {num_layers} layers, this may take a while...")
        
        try:
            # Use trimesh's built-in slice_plane method
            # This is more reliable than manual sectioning
            layers = []
            ...
            for i, z in enumerate(z_heights):
                ...
                # Convert to 2D path and get vertices
                try:
                    # to_planar returns the 2D path and transformation
                    path_2d, to_3D = slice_2d.to_planar()
                    
                    # Get discrete paths (list of closed polygons)
                            ...
                            if isinstance(paths, list):
                                for p in paths:
                                    if hasattr(p, '__len__') and len(p) > 1:
                                        path_list.append(np.asarray(p))
                            else:
                                # Single path
                                if hasattr(paths, '__len__') and len(paths) > 1:
                                    path_list.append(np.asarray(paths))
            ...
            if len(layers) == 0:
                ...
                print(f"   💡 Try: Reduce layer height to {self.layer_height/2:.1f}mm")
    ...
    def _extract_paths(self, path_2d, to_3D=None):
        """Extract printable paths from 2D slice and optionally transform to 3D coordinates
        
        Args:
            path_2d: The 2D planar path from trimesh
            to_3D: Transformation matrix to convert 2D coordinates back to 3D world space
        """
        paths = []
        
        # Primary method: use discrete property which gives us the actual coordinates
        if hasattr(path_2d, 'discrete'):
            try:
                # path_2d.discrete is a list of (n,2) arrays of 2D coordinates
                discrete_paths = path_2d.discrete
                if isinstance(discrete_paths, list):
                    for path in discrete_paths:
                        if hasattr(path, '__len__') and len(path) > 1:
                            path_array = np.array(path)
                            
                            # Transform back to 3D world coordinates if transform is available
                            if to_3D is not None:
                                # Convert 2D points to 3D using the transformation matrix
                                # Add Z=0 coordinate for 2D points, then transform
                                path_3d = np.column_stack([path_array, np.zeros(len(path_array))])
                                path_3d = trimesh.transformations.transform_points(path_3d, to_3D)
                                # Extract just X,Y from transformed 3D points
                                path_array = path_3d[:, :2]
                            
                            paths.append(path_array)
                elif hasattr(discrete_paths, '__len__') and len(discrete_paths) > 1:
                    # Single path
                    path_array = np.array(discrete_paths)
                ...
                print(f"      ⚠️  Failed to extract discrete paths: {e}")
        
        # Fallback: try entities with points method
        if len(paths) == 0 and hasattr(path_2d, 'entities'):
            for entity in path_2d.entities:
                try:
                    if hasattr(entity, 'points') and callable(entity.points):
                        # It's a method, call it with vertices
                        points = entity.points(path_2d.vertices)
                        if len(points) > 1:
                            paths.append(points)
                    elif hasattr(entity, 'discrete'):
                        # Entity has discrete representation
                        points = entity.discrete(path_2d.vertices)
                        if len(points) > 1:
                            paths.append(points)
                except Exception as e:
                    # Silently skip entities that fail - this is expected for some entity types
                    pass
